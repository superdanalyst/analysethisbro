        ...
        if 'Date' not in data.columns:
            ...
            if columns_to_forecast:
                ...
                for col in columns_to_forecast:
                    try:
                        decomposition = seasonal_decompose(train_data_monthly[col], model='additive', period=12)
                        decompositions_monthly[col] = decomposition
                        fig = decomposition.plot()
                        fig.suptitle(f'Seasonal Decomposition of {col}', fontsize=10)
                        st.pyplot(fig)
                    except Exception as e:
                ...
                for col in columns_to_forecast:
                    try:
                        ...
                        model = ExponentialSmoothing(ts, trend='additive', seasonal='additive', seasonal_periods=12)
                        fit = model.fit()
                        forecast = fit.forecast(1)
        
                        y_true = actuals_monthly[col].iloc[0]
                        y_pred = forecast.iloc[0]
        
                        mae = abs(y_true - y_pred)
                        rmse = np.sqrt((y_true - y_pred) ** 2)
                        mape = abs((y_true - y_pred) / y_true) * 100
        
                        accuracy_metrics_monthly[col] = {
                            'Actual': y_true,
                            'Forecast': y_pred,
                            'MAE': mae,
                            'RMSE': rmse,
                        ...
                        st.write(f"**{col}**")
                        st.write(f"Actual: {y_true:.2f}, Forecast: {y_pred:.2f}")
